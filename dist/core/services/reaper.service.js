...
export class ReaperService {
     ...
     */
    static async recoverStaleWorkersIfLeader(staleThresholdMs = 30000) {
        const result = await LockService.withLock('reaper:sweep', config.locking.holderId, () => this.recoverStaleWorkers(staleThresholdMs));
        return result ?? { offlineWorkersCount: 0, recoveredJobsCount: 0, skippedNotLeader: true };
    }
    static async recoverStaleWorkers(staleThresholdMs = 30000) {
        const thresholdDate = new Date(Date.now() - staleThresholdMs);
        const result = await withTransaction(async (client) => {
            // 1. Find dead workers
            const deadWorkersRes = await client.query(`SELECT id, hostname, pid
         FROM workers
         WHERE status = 'ONLINE'
           AND last_heartbeat_at < $1`, [thresholdDate]);
            if (deadWorkersRes.rows.length === 0) {
                return { offlineWorkersCount: 0, recoveredJobsCount: 0, deadWorkerIds: [] };
            }
         ...
         SET status = 'OFFLINE'
         WHERE id IN (${workerPlaceholders})`, deadWorkerIds);
            // 3. Find and recover orphaned jobs
            const orphanedJobsRes = await client.query(`SELECT id, queue_id, attempt_count, max_attempts
         FROM jobs
         WHERE locked_by IN (${workerPlaceholders})
           AND status IN ('CLAIMED', 'RUNNING')`, deadWorkerIds);
            ...
            for (const job of orphanedJobsRes.rows) {
             ...
             SET status = 'DEAD_LETTER',
                 ...
                 completed_at = NOW()
             WHERE id = $1`, [job.id]);
                    await client.query(`INSERT INTO dead_letter_queue (job_id, queue_id, failed_reason, total_attempts)
             VALUES ($1, $2, 'Worker timed out / crashed during execution', $3)
             ON CONFLICT (job_id) DO NOTHING`, [job.id, job.queue_id, job.attempt_count]);
                }
                recoveredCount++;
            }
            return {
...
//# sourceMappingURL=reaper.service.js.map