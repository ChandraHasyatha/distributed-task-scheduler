...
export class WorkerService {
    static async registerWorker(params) {
        ...
        const res = await query(`INSERT INTO workers (hostname, pid, status, concurrency_limit, active_jobs_count, shard_id, started_at, last_heartbeat_at)
       VALUES ($1, $2, 'ONLINE', $3, 0, $4, NOW(), NOW())
       RETURNING *`, [hostname, pid, concurrencyLimit, shardId]);
    ...
    static async recordHeartbeat(params) {
        ...
        await withTransaction(async (client) => {
            await client.query(`UPDATE workers
         SET last_heartbeat_at = NOW(),
             active_jobs_count = $1
         WHERE id = $2`, [activeJobsCount, workerId]);
            await client.query(`INSERT INTO worker_heartbeats (worker_id, cpu_usage_pct, memory_usage_mb, active_jobs_count, timestamp)
         VALUES ($1, $2, $3, $4, NOW())`, [workerId, cpuUsagePct, memoryUsageMb, activeJobsCount]);
        });
        eventBus.publish('WORKER_UPDATED', { workerId, activeJobsCount, cpuUsagePct, memoryUsageMb });
    }
    static async setWorkerStatus(workerId, status) {
    ...
    static async claimJobsFromQueue(params) {
        ...
        try {
            claimedJobs = await withTransaction(async (client) => {
                ...
                const qRes = await client.query(`SELECT id, concurrency_limit, is_paused, shard_count
           FROM queues
           WHERE id = $1
           FOR UPDATE`, [queueId]);
                ...
                const countRes = await client.query(`SELECT COUNT(*) as count
           FROM jobs
           WHERE queue_id = $1
             AND status IN ('CLAIMED', 'RUNNING')`, [queueId]);
                ...
                const shardFilter = shardCount > 1
                ...
                const claimRes = await client.query(`SELECT id
           FROM jobs
           WHERE queue_id = $1
             AND status = 'QUEUED'
             AND run_at <= NOW()
           ...
           LIMIT ${claimLimit}
           FOR UPDATE SKIP LOCKED`, [queueId]);
                ...
                const updatedJobs = await client.query(`UPDATE jobs
           SET status = 'CLAIMED',
               locked_by = $1,
               locked_at = NOW(),
               attempt_count = attempt_count + 1
           WHERE id IN (${placeholders})
           RETURNING *`, [workerId, ...jobIds]);
    ...
    static async listWorkers() {
        const res = await query('SELECT * FROM workers ORDER BY started_at DESC');
        return res.rows;
    }
}
