...
export class QueueService {
    static async createQueue(params) {
        const { projectId, name, priority = 10, concurrencyLimit = 5, retryPolicyId = null, shardCount = 1, } = params;
        const res = await query(`INSERT INTO queues (project_id, name, priority, concurrency_limit, retry_policy_id, shard_count)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [projectId, name, priority, concurrencyLimit, retryPolicyId, shardCount]);
        return res.rows[0];
    }
    static async getQueueById(id) {
        const res = await query('SELECT * FROM queues WHERE id = $1', [id]);
    ...
    static async listQueuesByProject(projectId) {
        const res = await query('SELECT * FROM queues WHERE project_id = $1 ORDER BY priority DESC, created_at ASC', [projectId]);
        const queuesWithStats = await Promise.all(res.rows.map(async (queue) => {
            const stats = await this.getQueueStats(queue.id);
            return { ...queue, stats };
        }));
        return queuesWithStats;
    }
    static async updateQueue(id, updates) {
        const fields = [];
        const values = [];
        let idx = 1;
        ...
        if (fields.length === 0) {
            const existing = await query('SELECT * FROM queues WHERE id = $1', [id]);
            return existing.rows[0] || null;
        }
        fields.push(`updated_at = NOW()`);
        values.push(id);
        const res = await query(`UPDATE queues SET ${fields.join(', ')} WHERE id = $${idx} RETURNING *`, values);
        if (res.rows[0]) {
            eventBus.publish('QUEUE_UPDATED', { queueId: id, ...updates });
        }
        return res.rows[0] || null;
    }
    static async setPaused(id, isPaused) {
        const res = await query(`UPDATE queues SET is_paused = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [isPaused, id]);
        if (res.rows[0]) {
    ...
    static async createRetryPolicy(params) {
        const res = await query(`INSERT INTO retry_policies (project_id, name, strategy, max_attempts, initial_interval_ms, max_interval_ms, backoff_factor)
    ...
    static async listRetryPolicies(projectId) {
        const res = await query('SELECT * FROM retry_policies WHERE project_id = $1 ORDER BY created_at ASC', [projectId]);
        return res.rows;
    }
}
