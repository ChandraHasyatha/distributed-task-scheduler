...
export class JobExecutor {
    static async execute(job, workerId) {
        ...
        await withTransaction(async (client) => {
         ...
         SET status = 'RUNNING',
             started_at = NOW()
         WHERE id = $1`, [job.id]);
            const execRes = await client.query(`INSERT INTO job_executions (job_id, worker_id, attempt_number, status, started_at)
         VALUES ($1, $2, $3, 'RUNNING', NOW())
         RETURNING id`, [job.id, workerId, job.attempt_count]);
        ...
        try {
            ...
            await withTransaction(async (client) => {
                await client.query(`UPDATE job_executions
           SET status = 'COMPLETED',
               finished_at = NOW(),
               duration_ms = $1
           WHERE id = $2`, [durationMs, executionId]);
                await client.query(`UPDATE jobs
           SET status = 'COMPLETED',
               completed_at = NOW(),
               locked_by = NULL,
    ...
    static async runTaskWithTimeout(job, executionId) {
        await this.appendLog(executionId, 'INFO', `Started executing task '${job.job_type}' (Attempt ${job.attempt_count}/${job.max_attempts})`);
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Execution timed out after ${job.timeout_ms}ms`)), job.timeout_ms));
    ...
    static async dispatchTaskHandler(job, executionId) {
    ...
    static async handleJobFailure(params) {
        ...
        await withTransaction(async (client) => {
         ...
         SET status = 'FAILED',
             ...
             error_message = $2,
             error_stack = $3
         WHERE id = $4`, [durationMs, errorMessage, errorStack, executionId]);
            ...
            if (job.retry_policy_id) {
                const polRes = await client.query('SELECT * FROM retry_policies WHERE id = $1', [job.retry_policy_id]);
                if (polRes.rows.length > 0) {
                    const p = polRes.rows[0];
                    strategy = p.strategy;
                    initialIntervalMs = p.initial_interval_ms;
                    maxIntervalMs = p.max_interval_ms;
                    backoffFactor = parseFloat(p.backoff_factor) || 2.0;
                }
            }
           ...
           SET status = 'DEAD_LETTER',
               ...
               locked_at = NULL
           WHERE id = $1`, [job.id]);
                const dlqRes = await client.query(`INSERT INTO dead_letter_queue (job_id, queue_id, failed_reason, total_attempts)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (job_id) DO UPDATE
           SET failed_reason = EXCLUDED.failed_reason, total_attempts = EXCLUDED.total_attempts
           RETURNING id`, [job.id, job.queue_id, errorMessage, job.attempt_count]);
        ...
        if (pendingAiSummary) {
            ...
            const payload = typeof job.payload === 'string' ? JSON.parse(job.payload) : job.payload;
            query('SELECT attempt_number, error_message FROM job_executions WHERE job_id = $1 ORDER BY attempt_number ASC', [job.id])
                .then((executionsRes) => AiSummaryService.summarizeFailure({
                dlqId,
    ...
    static async appendLog(executionId, level, message, data) {
        try {
            await query(`INSERT INTO job_logs (job_execution_id, level, message, data, logged_at)
         VALUES ($1, $2, $3, $4, NOW())`, [executionId, level, message, data ? JSON.stringify(data) : null]);
        }
        catch (err) {
