...
export class LockService {
  private static heldClients: Map<string, pg.PoolClient> = new Map();
  // In-memory fallback so the feature still demos correctly when running
  // against pg-mem (tests / no local Postgres available).
  private static memoryLocks: Set<string> = new Set();

  static async tryAcquire(lockKey: string, holder: string): Promise<boolean> {
    if (isMemoryDbMode()) {
  ...
  private static async logAcquire(lockKey: string, holder: string): Promise<void> {
    try {
      await query(
        `INSERT INTO distributed_lock_log (lock_key, holder) VALUES ($1, $2)`,
        [lockKey, holder]
      );
    } catch {
  ...
  private static async logRelease(lockKey: string): Promise<void> {
    try {
      await query(
        `UPDATE distributed_lock_log
         SET released_at = NOW()
         WHERE id = (
  ...
  static async getActiveLocks(): Promise<any[]> {
    const res = await query(
      `SELECT * FROM distributed_lock_log WHERE released_at IS NULL ORDER BY acquired_at DESC LIMIT 50`
    );
    return res.rows;
  }
}
