...
import { Job, JobExecution, JobLog, JobStatus, Queue } from '../types/index.js';
...
export class JobService {
  static async enqueueJob(params: EnqueueJobParams): Promise<Job> {
    const {
      queueId,
      jobType,
      payload = {},
    ...
    if (!maxAttempts || !retryPolicyId) {
      const qRes = await query<{ retry_policy_id: string | null; max_attempts?: number }>(
        `SELECT q.retry_policy_id, r.max_attempts
         FROM queues q
  ...
  static async enqueueBatch(queueId: string, jobs: Omit<EnqueueJobParams, 'queueId'>[]): Promise<Job[]> {
  ...
  static async getJobById(id: string): Promise<Job | null> {
    const res = await query<Job>('SELECT * FROM jobs WHERE id = $1', [id]);
  ...
  static async cancelJob(id: string): Promise<Job | null> {
    const res = await query<Job>(
      `UPDATE jobs
  ...
  static async getJobExecutions(jobId: string): Promise<JobExecution[]> {
    const res = await query<JobExecution>(
  ...
  static async getExecutionLogs(executionId: string): Promise<JobLog[]> {
    const res = await query<JobLog>(
