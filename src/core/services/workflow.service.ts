...
export class WorkflowService {
  static async addDependencies(jobId: string, dependsOnJobIds: string[]): Promise<void> {
  ...
  static async getDependencyGraph(jobId: string): Promise<{
    dependsOn: Job[];
    dependents: Job[];
  }> {
    const dependsOnRes = await query<Job>(
    ...
    const dependentsRes = await query<Job>(
      `SELECT j.* FROM jobs j
  ...
  static async isUnblocked(jobId: string): Promise<boolean> {
    const res = await query<{ id: string }>(
      `SELECT jd.id FROM job_dependencies jd
  ...
  static async promoteUnblockedJobs(): Promise<number> {
    ...
    const waitingRes = await query<{ id: string }>(
      `SELECT id FROM jobs WHERE status = 'WAITING'`
    );

    if (waitingRes.rows.length === 0) return 0;
  ...
  static async onJobCompleted(completedJobId: string): Promise<string[]> {
    const candidatesRes = await query<{ id: string }>(
      `SELECT j.id FROM jobs j
       JOIN job_dependencies jd ON jd.job_id = j.id
       WHERE jd.depends_on_job_id = $1 AND j.status = 'WAITING'`,
      [completedJobId]
    );
