...
export class AiSummaryService {
  static async summarizeFailure(params: {
    ...
    errorHistory?: { attempt: number; error: string | null }[];
  }): Promise<string> {
    ...
    await query(
      `UPDATE dead_letter_queue
       ...
       WHERE id = $2`,
      [summary, params.dlqId]
    );

    return summary;
  }

  private static async summarizeWithClaude(params: {
    jobType: string;
    payload: Record<string, any>;
    failedReason: string;
    totalAttempts: number;
...
In 2-3 sentences, explain the likely root cause and suggest one concrete next step for an on-call engineer. Be specific and concise.`;
  ...
  private static summarizeHeuristically(params: {
    jobType: string;
    failedReason: string;
    totalAttempts: number;
    errorHistory?: { attempt: number; error: string | null }[];
