...
export class WebhookService {
  static async createTrigger(params: {
    ...
    const res = await query<WebhookTrigger>(
      `INSERT INTO webhook_triggers (project_id, queue_id, name, signing_secret, job_type, default_priority)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [params.projectId, params.queueId, params.name, signingSecret, params.jobType, params.defaultPriority ?? 10]
    );
    return res.rows[0];
  }

  static async listTriggers(projectId: string): Promise<WebhookTrigger[]> {
    const res = await query<WebhookTrigger>(
      'SELECT * FROM webhook_triggers WHERE project_id = $1 ORDER BY created_at DESC',
      [projectId]
    );
    return res.rows;
  }

  static async getTrigger(id: string): Promise<WebhookTrigger | null> {
    const res = await query<WebhookTrigger>('SELECT * FROM webhook_triggers WHERE id = $1', [id]);
   ...
   */
  static async fire(trigger: WebhookTrigger, eventPayload: Record<string, any>): Promise<string> {
    await query(
