...
import { User, Organization, OrganizationMembership, UserRole } from '../types/index.js';
...
export class AuthService {
  static async registerUserWithOrg(params: {
    ...
    orgName: string;
    orgSlug?: string;
  }): Promise<{ user: User; organization: Organization; membership: OrganizationMembership }> {
    ...
    return withTransaction(async (client) => {
      ...
      const memRes = await client.query<OrganizationMembership>(
  ...
  static async getUserMemberships(userId: string): Promise<
    (OrganizationMembership & { organization_name: string; organization_slug: string })[]
  > {
